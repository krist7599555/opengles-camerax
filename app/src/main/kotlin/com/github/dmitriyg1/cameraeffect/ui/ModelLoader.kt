...
class ModelLoader(val assets: AssetManager, val folder: String) {
      ...
      when (cmd) {
        ...
        "f" -> {
          ...
          for (_vertex in vertices) {
            ...
            val idxPosition: Int? = try { vertex[0].toInt() - 1 } catch(e: NumberFormatException) { null }
            val idxNormal  : Int? = try { vertex[1].toInt() - 1 } catch(e: NumberFormatException) { null }
            val idxTexture : Int? = try { vertex[2].toInt() - 1 } catch(e: NumberFormatException) { null }
  ...
  private fun parseMTL(source: String): HashMap<String, ModelLoader.Material> {
    val materials = HashMap<String, ModelLoader.Material>();
    var currentMaterial: ModelLoader.Material? = null
    for (_line in source.lines()) {
      val line = _line.trim().split(" ")
      val cmd = line.first()
